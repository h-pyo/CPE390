...
uint64_t f2c(uint64_t n) {
  uint64_t sum = 0;
  for (uint64_t i = 1; i <= n; i++) sum += i / (i + 1);
  return sum;
}
...
/*
        ...
        In other words, read the same number of elements as f4, but a bit out of
   order
*/
...
void xor_crypt(char msg[], uint64_t len, char key) {
  for (uint64_t i = 0; i < len; i++) msg[i] ^= key;
}
...
uint64_t crc32fast_intel(uint64_t crc, uint64_t p[], uint64_t n) {
  for (uint64_t i = 0; i < n; i++) crc = _mm_crc32_u64(crc, p[i]);  // use Intel
  return crc;
}
